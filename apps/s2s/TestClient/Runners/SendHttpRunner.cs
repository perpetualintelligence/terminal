        ...
        private async Task SendHttpCommandsAsync(HttpClient client, string serverAddress, CancellationToken cancellationToken)
        {
            try
            {
                ...
                foreach (var command in commands)
                {
                    ...
                    var response = await client.SendSingleToTerminalAsync(command, TerminalIdentifiers.RemoteCommandDelimiter, TerminalIdentifiers.RemoteBatchDelimiter, cancellationToken);
                ...
                var batchResponse = await client.SendBatchToTerminalAsync(commands, TerminalIdentifiers.RemoteCommandDelimiter, TerminalIdentifiers.RemoteBatchDelimiter, cancellationToken);
