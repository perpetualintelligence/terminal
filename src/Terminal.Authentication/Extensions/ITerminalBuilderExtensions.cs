...
namespace PerpetualIntelligence.Terminal.Extensions
{
    ...
    public static class ITerminalBuilderExtensions
    {
        ...
        /// <typeparam name="TFactory">The authentication application factory.</typeparam>
        ...
        /// <remarks>
        /// Use <see cref="MsalTokenNoCache"/> if your application does not require token caching.
        /// </remarks>
        public static ITerminalBuilder AddMsal<TProvider, TFactory, TAppCache, TDelegateHandler>(this ITerminalBuilder builder, string name, string? baseAddress = null, int? timeout = 120000) where TProvider : class, IAuthenticationProvider where TFactory : class where TAppCache : class, IMsalTokenCache where TDelegateHandler : DelegatingHandler
        {
            builder.Services.AddSingleton<IAuthenticationProvider, TProvider>();

            builder.Services.AddSingleton<IMsalTokenCache, TAppCache>();
