...
namespace OneImlx.Terminal.Extensions
{
    public class ITerminalBuilderExtensionsTests
    {
        ...
        {
            terminalBuilder.DefineCommand<MockCommandRunner>("id1", "name1", "desc", CommandType.Root, CommandFlags.Secured).Checker<MockCommandChecker>().Add();

            IServiceProvider serviceProvider = terminalBuilder.Services.BuildServiceProvider();
            CommandDescriptor cmd = serviceProvider.GetRequiredService<CommandDescriptor>();

            cmd.Type.Should().Be(CommandType.Root);
            cmd.Flags.Should().Be(CommandFlags.Secured);
        }

        [Fact]
        public void AddCommandDescriptorWithSpecialAnnotationsFlagsShouldNotError()
        {
            terminalBuilder.DefineCommand<MockCommandRunner>("id1", "name1", "desc", CommandType.Group, CommandFlags.Secured | CommandFlags.Obsolete).Checker<MockCommandChecker>().Add();

            IServiceProvider serviceProvider = terminalBuilder.Services.BuildServiceProvider();
            CommandDescriptor cmd = serviceProvider.GetRequiredService<CommandDescriptor>();

            cmd.Type.Should().Be(CommandType.Group);
            cmd.Flags.Should().Be(CommandFlags.Secured | CommandFlags.Obsolete);
        }
        ...
        {
            ICommandBuilder commandBuilder = terminalBuilder.DefineCommand<MockCommandRunner>("id1", "name1", "description1", CommandType.Root, CommandFlags.Secured).Checker<MockCommandChecker>();
            ...
            instance.Flags.Should().Be(CommandFlags.Secured);
        }
