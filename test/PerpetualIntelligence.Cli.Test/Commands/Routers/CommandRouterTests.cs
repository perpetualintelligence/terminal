...
namespace PerpetualIntelligence.Cli.Commands.Routers
{
    ...
    public class CommandRouterTests : InitializerTests
    {
        ...
        public async Task RouterShouldErrorOnNoLicense()
        {
            ...
            CommandRouterContext routerContext = new("test_command_string");
            await TestHelper.AssertThrowsErrorExceptionAsync(() => router.RouteAsync(routerContext), "invalid_license", "The license is not yet extracted. Please ensure you use the CLI hosted service. service=PerpetualIntelligence.Cli.Integration.CliHostedService");
